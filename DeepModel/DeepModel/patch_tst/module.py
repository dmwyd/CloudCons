...
class PatchTSTModel(nn.Module):
    ...
    def __init__(
        ...
        # )
        # self.patch_proj = nn.Linear(patch_len + 2 + self.num_feat_dynamic_real * patch_len, d_model)
        self.patch_proj = nn.Linear(patch_len + self.num_feat_dynamic_real * patch_len, d_model)

        # if bool(torch.isnan(self.patch_proj.weight.data.norm())):
    ...
    def _forward_internal(
        ...
        past_target_patches = past_target_scaled.unfold(dimension=1, size=self.patch_len, step=self.stride)
    ...
    def loss(
        ...
        distr_args, loc, scale = self._forward_internal(
            past_target=past_target,
            past_observed_values=past_observed_values,
            past_time_feat=past_time_feat,
            future_time_feat=future_time_feat,
